...
async function runSync(label, { deadlineMs = 120_000 } = {}) {
  ...
  try {
    ...
    console.log("[reconcileSpace THREW]", thrown);
  }
  ...
  return { tally, pages, elapsed, thrown, newWarns: warns.slice(warnsBefore), states, watch: w };
}
...
check(`PHASE A: 25 UNCONFIGURED ops now trip the incremental cap`,
  a.newWarns.some((w) => w.includes("bounded operation limit")),
  a.newWarns.join(" | ").slice(0, 200) || "no warns");
check("PHASE A: and the engine answers with a FULL CAR download",
  (a.tally["com.atproto.space.getRepo"] ?? 0) >= 1,
  JSON.stringify(a.tally));
check("PHASE A: still ingests ZERO records — the whole repo was downloaded for nothing",
  rsvpRows === 0, `isolated_records_rsvp rows = ${rsvpRows}, ops paged = ${aOps}`);
check("PHASE A: checkpoint still advanced (recovery healed it)", revA !== rev0, `${rev0} -> ${revA}`);
...
check("PHASE B: the deadline is now ENFORCED inside incrementalRepo",
  b.elapsed <= DEADLINE_MS + 1_500,
  `elapsed ${b.elapsed} ms vs ${DEADLINE_MS} ms deadline (${b.pages.length} slow page(s), ${bOps} ops)`);
check("PHASE B: enforcement is a THROW out of reconcileSpace, not a clean stop",
  b.thrown !== null, b.thrown ?? "did not throw");
check("PHASE B: the space watch row is left marked with an error string",
  Boolean(b.watch?.error), JSON.stringify({ status: b.watch?.status, error: b.watch?.error, nextReconcileAt: b.watch?.nextReconcileAt }));
